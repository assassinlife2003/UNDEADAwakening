...
#Player2 FirstPerson Graphics
    ...
    crosshair.moveTo(mouse.x - 89,fp.y - 105)
    if crosshair.collidedWith(zombie3[index]) and mouse.LeftClick:
        zombie3[index].health -= 50
        zombielevel2complete += 1
        ammocount -= 2

    if fp.collidedWith(zombie3[index]):
        p2.health -= 20

    if zombie3[index].health < 1:
    ...
    game.drawText("Health: " + str(p2.health),0,0,Font(white,24,black))
    game.drawText("Ammo: " + str(ammocount),20,20,Font(white,24,black))
    game.update(30)
